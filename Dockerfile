...
# Install pipx and uv (modern Python dependency manager)
RUN pip install --no-cache-dir pipx && pipx ensurepath && pipx install uv
...
RUN uv pip install -r <(uv pip compile pyproject.toml) --system --no-cache-dir
